...
# Matching-only normalization map.
# This is intentionally small and high-confidence.
MATCH_NORMALIZATION_MAP = {
    r'\bACAD\b': 'ACADEMY',
    r'\bSCH\b': 'SCHOOL',
    r'\bINST\b': 'INSTITUTE',
    r'\bCTR\b': 'CENTER',
...
def load_files():
    file1 = pd.read_csv('final_addr2.csv', dtype=CEEB_DTYPE , keep_default_na=False)
    file2 = pd.read_csv('CAPPEX_DB.DBT.B_CPX_REFERENCE_DATA_HIGH_SCHOOL.csv', dtype=CEEB_DTYPE , keep_default_na=False)
    file3 = pd.read_excel('HS Ceeb Code file_PAPA.xlsx', dtype=CEEB_DTYPE, engine='openpyxl' , keep_default_na=False)
    file4 = pd.read_csv('CROSS_TENANT_DB.COMMON.REF__HIGH_SCHOOLS.csv', dtype=CEEB_DTYPE , keep_default_na=False)
...
def normalize_school_name_for_match(name):
    ...
    This is separate from final cleanup.
    Allows obvious abbreviation equivalence like ACAD -> ACADEMY
    so consensus clusters form more reliably.
    """
    if pd.isna(name):
        return None
    ...
    for pattern, replacement in MATCH_NORMALIZATION_MAP.items():
...
def score_candidate(entry, support_count):
    ...
    Order:
    1. more exact support for same match_name
    2. better source priority
    3. prefer non-abbreviation
    4. prefer fuller original name
    """
...
def get_best_school_name(row, school_name_cols, threshold=90):
    ...
    if len(distinct_match_names) == 1:
        best_entry = max(
            entries,
            key=lambda e: score_candidate(e, support_count=1)
        )
        return best_entry['original_name']

    # IMPORTANT CHANGE:
    # Do not short-circuit to SC before consensus/clustering.
    # Let the cluster win first.
...
def clean_final_school_name(name):
    """
    Post-selection cleanup only.
    This does not affect matching except via the separate
    matching normalization function above.
    """
    if pd.isna(name):
        return name
...
def main():
    ...
    # Save outputs
    result_df.to_csv('merged_schools_new_1_12_changes.csv', index=False)
    result_df.to_excel('merged_schools_new_1_12_changes.xlsx', index=False, engine='openpyxl')
